    ...
    return (
        <Link href={"/movie/" + movie.id} className='group [perspective:1000px]'>
            <div className='  [transform-style:preserve-3d] transition-all duration-500 delay-25 group-hover:[transform:rotateY(180deg)] w-52 flex flex-col overflow-hidden'>
                ...
                <p className='absolute px-5 bg-black/90 opacity-0 group-hover:opacity-100 [transform:rotateY(180deg)] delay-300 duration-300 transition-opacity flex justify-center items-center text-center container inset-0 text-xl'>{truncateText(movie.description, 175)}</p>
            </div>  
        </Link>
    )
}
